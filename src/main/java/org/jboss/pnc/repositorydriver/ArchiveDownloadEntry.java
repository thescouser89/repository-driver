...
public class ArchiveDownloadEntry {
    ...
    public static ArchiveDownloadEntry fromTrackedContentEntry(RepositoryArtifact dto) {
        return new ArchiveDownloadEntry(
                getStoreKeyFromTargetRepository(dto.getTargetRepository()),
                dto.getDeployPath(),
                dto.getMd5(),
                dto.getSha256(),
                dto.getSha1(),
     ...
     */
    private static StoreKey getStoreKeyFromTargetRepository(TargetRepository targetRepository) {
        String[] split = targetRepository.getRepositoryPath().split("/");
        if (split.length <= 2) {
